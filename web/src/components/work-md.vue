	   	...
	   	<div class="selects">
	        <ul>
	        	<li @click="changeTab(-1)">
	        		<span :class="{nowsle:selected==-1}">全部</span>
	        	</li>
	            <li v-for="(num,index) in nowType" @click="changeTab(index)">
	                <span :class="{nowsle:selected==index}">{{num}}</span>
	            </li>
	        </ul>
	    </div>
        <ul class="cards">
...
.work-content{
	...
	.selects{
		...
		li{
			float: left;
			span{
				@include fonts(14px,#787878,42px);
				padding: 0 20px;
				cursor: pointer;
			}
			text-align: center;
